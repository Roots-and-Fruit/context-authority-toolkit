...
- Term single output includes semantic microdata redundancy:
  ...
  - Visible lead: when `cat_tooltip_content` is non-empty and the body does not already start with that same text (whitespace-normalized), `Cat_Term_Single_Chrome::render_lead_html()` prints it first inside the description wrapper. JSON-LD `description` stays tooltip-owned; tooltip is never copied into `post_content`; excerpt is not used as schema description. One-time tooltip migration copies legacy `post_content` only when tooltip meta is empty **and** that content has no Gutenberg `<!-- wp:` delimiters (term-section scaffolds must not become hovercard text). Stored tooltips that already contain block markup are scrubbed empty. `sanitize_tooltip_meta()` still preserves intentional plain-text characters such as `<strong>` when no block delimiters are present.
  - Visible aliases: “Also known as” is panel-only via `Cat_Term_Single_Chrome::render_aliases_html()` inside `render_panel_html()` when `cat_term_panel_show_aliases` is on. Peacekeeper must not inject aliases into the DefinedTerm article. JSON-LD `alternateName` / `termCode` still come from `get_display_aliases()`; empty alternatives omit both the panel line and those schema keys.
  - Visible related terms: `Cat_Term_Single_Chrome::render_related_html()` prints a Related terms list of permalinks inside the DefinedTerm article, outside `itemprop="description"`. Read-time filtering skips drafts, trash, non-term, unpublished, missing, and self. Empty related lists omit the block.
  ...
  - Panel placement (`Cat_Term_Panel` only decides where/whether to print):
    ...
    - `cat-toolkit/term-panel` is a dynamic block with `block.json` + editor script so Site Editor (Edit Site) does not treat it as missing. `edit` is a placeholder; frontend HTML still comes from `render_panel_block()` / chrome.
  - Term body scaffold (`Cat_Term_Section_Block`):
    - Block `cat-toolkit/term-section` wraps inner blocks in `<section class="cat-term-section" data-cat-section="{slot}">` with an H2. Slots are `what`, `how`, `examples`, `mistakes`, `takeaways`. Inner blocks are unrestricted. Group layout/flex/spacing supports stay off in v1.
    - H2: non-empty `customHeading` (after `sanitize_text_field`) wins; otherwise PHP prints the translated default via `esc_html__()` (`What it is`, `How it works`, `Examples`, `Common mistakes`, `Key takeaways`). Persist the slot key in `post_content`, never the default heading string. Editor labels use `wp.i18n.__()` plus `wp_set_script_translations`. Custom headings are editor-owned content.
    - Inserter pattern `cat-toolkit/term-page` (category `cat-toolkit`, `inserter` true) inserts five empty sections in slot order. Do not set `templateTypes` to `single` or `posts`.
    - CPT `template` + `template_lock` false seeds those five sections on **new** terms only. Existing terms are not rewritten. This is body scaffolding, not the FSE `single-term` layout (the “do not use `register_post_type( template )`” rule still applies to that layout).
    - Tooltip lead (`cat_tooltip_content`) is the intro paragraph with no H2. Related terms stay in existing meta/chrome (`seeAlso` / panel), not a sixth body section. This block must not emit FAQPage, HowTo, Article, Speakable, or LearningResource schema.
