...
- Term single output includes semantic microdata redundancy:
  ...
  - Panel placement (`Cat_Term_Panel` only decides where/whether to print):
    ...
    - Block themes (`wp_is_block_theme()`): skip classic sidebar/content injection. Ship block pattern `cat-toolkit/term-panel` (dynamic block `cat-toolkit/term-panel`) for opt-in insert. Do not ship `single-term.html` or edit theme files.
